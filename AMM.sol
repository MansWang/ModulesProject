...
contract AMM is AccessControl{
	...
	*/
	function tradeTokens( address sellToken, uint256 sellAmount ) public {
		...
		require(amountInWithFee > 0, "Trade amount too small");
		...
		if( sellToken == tokenA ) {
			...
			require(swapAmt > 0, "Swap amount is zero");
			require(ERC20(tokenB).balanceOf(address(this)) >= swapAmt, "Insufficient tokenB");

			require(ERC20(tokenB).transfer(msg.sender, swapAmt), "tokenB transfer failed");
			...
			require(swapAmt > 0, "Swap amount is zero");
			require(ERC20(tokenA).balanceOf(address(this)) >= swapAmt, "Insufficient tokenA ");

			require(ERC20(tokenA).transfer(msg.sender, swapAmt), "tokenA transfer failed");
	...
	*/
	function provideLiquidity( uint256 amtA, uint256 amtB ) public {
		...
		//YOUR CODE HERE
		if( amtA > 0 ) {
			require(ERC20(tokenA).transferFrom(msg.sender, address(this), amtA), "tokenA transferFrom failed");
		}
		if( amtB > 0 ) {
			require(ERC20(tokenB).transferFrom(msg.sender, address(this), amtB), "tokenB transferFrom failed");
		}
